...
function getCardItems(lang: Lang): ProjectCard[] {
  return projects.map((p) => ({
    id: p.slug,
    title: p.title,
    description: lang === 'fr' ? p.description : p.descriptionEn,
    href: p.githubUrl,
    tag: p.category,
    icon: p.icon,
    tags: p.tags,
    ...
    longDesc: lang === 'fr' ? p.longDesc : p.longDescEn,
  }))
}
...
export default function Projects() {
  ...
  const cardItems = getCardItems(lang)
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <p className="text-slate-500 text-sm mt-3">
          {lang === 'fr'
            ...
            : `Hover the active card to reveal · swipe or click to navigate · ${cardItems.length} projects`}
        </p>
      </div>
      ...
      <div className="mt-12">
        <CardStack<ProjectCard>
          ...
          renderCard={(item, { active }) => (
            <ProjectCardContent item={item} active={active} lang={lang} />
          )}
        />
      </div>
...
function ProjectCardContent({ item, active, lang }: { item: ProjectCard; active: boolean; lang: Lang }) {
    ...
    >
      ...
      >
        ...
        <div className="text-center">
          ...
          <span className="text-xs font-mono tracking-widest uppercase mt-1.5 block" style={{ color: item.languageColor }}>
            {item.tag}
          </span>
        </div>
        {item.featured && (
      ...
      >
        ...
        <div className="flex flex-wrap gap-1.5">
          {item.tags.slice(0, 5).map((tag) => (
            <span key={tag} className="px-2 py-0.5 rounded-full text-xs font-mono bg-white/10 border border-white/10 text-slate-300">
              {tag}
            </span>
          ))}
          {item.tags.length > 5 && (
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-white/10">
          <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: item.languageColor }} />
